...
1. **`bracket-seeding-labels`** _(next)_ — render a compact seed badge (`1E` / `2A` / `3C` = group-finish position + group letter) next to each **resolved** team in the **R32** column. The engine already emits the underlying labels (`lib/engine/bracket.ts`); only `app/components/Bracket.tsx` rendering changes. R32-only — later rounds are "Winner of M…" and carry no seed.
