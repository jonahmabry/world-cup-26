...
The third-place card capability models the World Cup third-place play-off (M103) — the one knockout
fixture contested by the two Semi-final **losers** rather than their winners — and renders it as a
detached card in the projected bracket. It introduces the `ThirdPlace` round, the M103 schedule entry,
and the `loser-of` slot reference (parallel to `winner-of`), the data model reused by the match-schedule
section.
