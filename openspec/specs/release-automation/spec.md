...
Releases are versioned, changelogged, tagged, and published automatically from per-change
changeset files rather than by hand, so a version bump, a git tag, or a GitHub Release can
never be forgotten. The package is private and is never published to a registry — release
automation produces git tags and GitHub Releases only.
